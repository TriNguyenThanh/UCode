...
import type { Route } from './+types/teacher.class.$classId.attendance.$sessionId.edit'
...
import { updateAttendanceSession, getAttendanceSessionById } from '~/services/attendanceService'
import type { ApiResponse, Class, AttendanceSession } from '~/types'
...
export default function EditAttendanceSession() {
  const { classData, session } = useLoaderData<typeof clientLoader>()
  ...
  // IP Check settings
  const [requireIpCheck, setRequireIpCheck] = useState(session.requireIpCheck)
  const [allowedIpSubnet, setAllowedIpSubnet] = useState(session.allowedIpSubnet || '')
  
  // GPS Check settings
  const [requireGpsCheck, setRequireGpsCheck] = useState(session.requireGpsCheck)
  const [allowedLatitude, setAllowedLatitude] = useState(session.allowedLatitude?.toString() || '')
  const [allowedLongitude, setAllowedLongitude] = useState(session.allowedLongitude?.toString() || '')
  const [allowedRadiusMeters, setAllowedRadiusMeters] = useState(session.allowedRadiusMeters?.toString() || '100')
  
  // Face Check setting
  const [requireFaceCheck, setRequireFaceCheck] = useState(session.requireFaceCheck)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Không thể cập nhật phiên điểm danh')
    } finally {
      setLoading(false)
    }
  }
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        ...
        <Collapse in={success}>
          <Alert severity="success" sx={{ mb: 3 }}>
            Cập nhật phiên điểm danh thành công! Đang chuyển hướng...
          </Alert>
        </Collapse>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Paper sx={{ p: 3, mb: 3 }}>
              ...
              <TextField
                ...
                required
                disabled
                helperText="Mã phiên không thể thay đổi sau khi tạo"
                sx={{ mb: 2 }}
              />
